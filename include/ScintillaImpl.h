...
public:
	...
	Line MarkerLineFromHandle(int markerHandle) { return Call(SCI_MARKERLINEFROMHANDLE, markerHandle); }
	...
	Position BraceMatchNext(Position pos, Position startPos) { return Call(SCI_BRACEMATCHNEXT, pos, startPos); }
	...
	Position IndicatorEnd(int indicator, Position pos) { return Call(SCI_INDICATOREND, indicator, pos); }
	Position IndicatorStart(int indicator, Position pos) { return Call(SCI_INDICATORSTART, indicator, pos); }
	...
	int EOLAnnotationGetStyle(Line line) { return Call(SCI_EOLANNOTATIONGETSTYLE, line); }
	int EOLAnnotationGetStyleOffset() { return Call(SCI_EOLANNOTATIONGETSTYLEOFFSET); }
	int EOLAnnotationGetText(Line line, char* text) { return Call(SCI_EOLANNOTATIONGETTEXT, line, reinterpret_cast<intptr_t>(text)); }
	int EOLAnnotationGetVisible() { return Call(SCI_EOLANNOTATIONGETVISIBLE); }
	...
	int MarkerHandleFromLine(Line line, int which) { return Call(SCI_MARKERHANDLEFROMLINE, line, which); }
	int MarkerNumberFromLine(Line line, int which) { return Call(SCI_MARKERNUMBERFROMLINE, line, which); }
	...
	void CallTipSetHlt(Position highlightStart, Position highlightEnd) { Call(SCI_CALLTIPSETHLT, highlightStart, highlightEnd); }
	...
	void EOLAnnotationClearAll() { Call(SCI_EOLANNOTATIONCLEARALL); }
	void EOLAnnotationSetStyle(Line line, int style) { Call(SCI_EOLANNOTATIONSETSTYLE, line, style); }
	void EOLAnnotationSetStyleOffset(int style) { Call(SCI_EOLANNOTATIONSETSTYLEOFFSET, style); }
	void EOLAnnotationSetText(Line line, const char* text) { Call(SCI_EOLANNOTATIONSETTEXT, line, reinterpret_cast<intptr_t>(text)); }
	void EOLAnnotationSetVisible(int visible) { Call(SCI_EOLANNOTATIONSETVISIBLE, visible); }
